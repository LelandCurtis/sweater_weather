class OpenWeatherMapFacade
  class << self
    ...
    def get_forecast(lat, lon)
      ...
      hourly_weather = json[:hourly].map do |hour|
        HourlyWeather.new(clean_hourly_weather_data(hour))
      end
      daily_weather = json[:daily].map do |day|
    ...
    def clean_current_weather_data(data)
      ...
      output_data[:conditions] = data[:weather][:description]
      output_data[:icon] = data[:weather][:icon]
      output_data
    end
